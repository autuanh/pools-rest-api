...
class TestAPI(TestCase):
    ...
    def test_insert_to_new_pool(self):
        ...
        Expected outputs:
            A post request to create a new pool with pool id 97 and pool values of [1, 2, 3, 4, 5]
            should return status code 200 with message 'inserted'.
        """
        response = self.client.post(
            "/pools/append",
            json={"poolId": 97, "poolValues": [1, 2, 3, 4, 5]},
    ...
    def test_append_to_existing_pool(self):
        ...
        Expected outputs:
            - The first post request should create a new pool with pool id 98 and pool values of [11, 12, 13, 14]
            and return status code 200 with message 'inserted'.
            - The second post request should append pool values of [15, 16, 17, 18] to pool id 98
            and return status code 200 with message 'appended'.
        """

        response_1 = self.client.post(
            "/pools/append",
            json={"poolId": 98, "poolValues": [11, 12, 13, 14]},
            headers={"Content-Type": "application/json"},
        )

        response_2 = self.client.post(
            "/pools/append",
            json={"poolId": 98, "poolValues": [15, 16, 17, 18]},
            headers={"Content-Type": "application/json"},
        )
    ...
    def test_query_pool_happy_case(self):
        ...
        Expected outputs:
            - The first post request should create a new pool with pool id 99 and pool values of [1, 2, 3, 4, 5]
            and return status code 200 with message 'inserted'.
            - The second post request to query for pool id 99 and 50th percentile should return status code 200,
            with calculated percentile value is 5 and total count of values in pool is 5.
        """
        response_v1 = self.client.post(
            "/pools/append",
            json={"poolId": 99, "poolValues": [1, 3, 5, 7, 9]},
            headers={"Content-Type": "application/json"},
        )
